...
class FlashCardTest(StageTest):
    def generate(self):
        return [TestCase(stdin="3", check_function=FlashCardTest.check_menu_and_exit),
                TestCase(stdin="1\n2\n3", check_function=FlashCardTest.check_sub_menu_and_exit),
    ...
    @staticmethod
    def check_menu_and_exit(reply, attach):
        reply = reply.lower()
        if MAIN_MENU.lower().strip() not in reply:
            return CheckResult.wrong("Your program doesn't output the starting message in the required format:\n"
                                     "{0}".format(MAIN_MENU))
        if GOODBYE_MESSAGE.lower() not in reply:
            return CheckResult.wrong("Your program doesn't output the message \"{0}\" "
                                     "after the 'Exit' command".format(GOODBYE_MESSAGE))
        return CheckResult.correct()

    @staticmethod
    def check_sub_menu_and_exit(reply, attach):
        reply = reply.lower()
        if SUB_MENU.lower() not in reply:
            return CheckResult.wrong("Your program doesn't output the sub-menu message in the required format:\n"
                                     "{0}".format(SUB_MENU))
        if GOODBYE_MESSAGE.lower() not in reply:
            return CheckResult.wrong("Your program doesn't output the message \"{0}\" "
                                     "after the 'Exit' command".format(GOODBYE_MESSAGE))
